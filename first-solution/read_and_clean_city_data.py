...
CONFIG = {
    'input_file': os.path.join(SCRIPT_DIR, 'sources', 'german_cities.xlsx'),
...
def save_df_as_csv(df, file) -> None:
    ...
    output_file = os.path.join(SCRIPT_DIR, 'results', 'german_cities_clean_utf8.csv')
    cols = list(df.columns)
    if 'city_type' in cols:
        cols.insert(cols.index('city_name') + 1, cols.pop(cols.index('city_type')))
    
    df[cols].to_csv(output_file, index=False, sep=',', decimal='.', encoding='utf-8')
...
def start_cleaning():
    ...
    if raw_df is not None:
        ...
        save_df_as_csv(cleaned_df, CONFIG['input_file'])
