...
-------------------------
Synthesizes a weighted demographic dataset for Germany based on real population data.

Key Features:
* Tiered Distribution: Weights customers across Metropolises, Mid-sized Cities, and Rural zones.
* Validated GIS: Constraints generation to verified pgeocode-backed postal codes.
* Spatial Sprawl: Implements a radius-based variance to simulate realistic urban distribution.
* Data Integrity: Aggregates individual records into a PLZ-keyed demand matrix for optimization.

Ensures the 'Demand Side' of the model is statistically significant and geographically accurate.

Coreflow
1. Reference Load	> get_valid_german_plzs	> Fetches unique, geocoded PLZs from pgeocode.
2. Tiered Allocation > generate_customer_data >	Calculates customer quotas per city based on population.
3. Spatial Jitter >	get_real_nearby_plz	> Randomly offsets PLZs within a defined search radius.
4. Aggregation	> groupby >	Collapses individual customers into a PLZ-level count.
